...
function detailFromDraft(draft) {
  ...
  return {
    ...
    target_audience: draft.target_audience || draft.targetAudience || existing.target_audience || existing.targetAudience || "adult_men",
    targetAudience: draft.targetAudience || draft.target_audience || existing.targetAudience || existing.target_audience || "adult_men",
    male_attention_score: draft.male_attention_score || draft.maleAttentionScore || existing.male_attention_score || existing.maleAttentionScore || 0,
    maleAttentionScore: draft.maleAttentionScore || draft.male_attention_score || existing.maleAttentionScore || existing.male_attention_score || 0,
    dm_trigger_score: draft.dm_trigger_score || draft.dmTriggerScore || existing.dm_trigger_score || existing.dmTriggerScore || 0,
    dmTriggerScore: draft.dmTriggerScore || draft.dm_trigger_score || existing.dmTriggerScore || existing.dm_trigger_score || 0,
    comment_trigger_score: draft.comment_trigger_score || draft.commentTriggerScore || existing.comment_trigger_score || existing.commentTriggerScore || 0,
    commentTriggerScore: draft.commentTriggerScore || draft.comment_trigger_score || existing.commentTriggerScore || existing.comment_trigger_score || 0,
...
function clientDraft(rowOrDraft) {
  if (rowOrDraft?.text && rowOrDraft?.score_total !== undefined) {
    ...
    return {
      ...
      target_audience: detail.target_audience || detail.targetAudience || "adult_men",
      targetAudience: detail.targetAudience || detail.target_audience || "adult_men",
      male_attention_score: detail.male_attention_score || detail.maleAttentionScore || 0,
      maleAttentionScore: detail.maleAttentionScore || detail.male_attention_score || 0,
      dm_trigger_score: detail.dm_trigger_score || detail.dmTriggerScore || 0,
      dmTriggerScore: detail.dmTriggerScore || detail.dm_trigger_score || 0,
      comment_trigger_score: detail.comment_trigger_score || detail.commentTriggerScore || 0,
      commentTriggerScore: detail.commentTriggerScore || detail.comment_trigger_score || 0,
...
function fallbackRomanceDrafts(researchId, options = {}, diagnostics = {}) {
  ...
  return rows.map((row) => {
    const detail = {
      ...
      targetAudience: "adult_men",
      male_attention_score: row.genre === "sns_love" ? 90 : 86,
      maleAttentionScore: row.genre === "sns_love" ? 90 : 86,
      dm_trigger_score: row.genre === "sns_love" ? 88 : 82,
      dmTriggerScore: row.genre === "sns_love" ? 88 : 82,
      comment_trigger_score: row.genre === "romance_aruaru" ? 90 : 84,
      commentTriggerScore: row.genre === "romance_aruaru" ? 90 : 84,
      viral_score: row.score,
      viralScore: { curiosity: 78, nostalgia: 65, surprise: 70, empathy: 90, controversy: 60, commentability: 88, male_attention: row.genre === "sns_love" ? 90 : 86, dm_trigger: row.genre === "sns_love" ? 88 : 82, comment_trigger: row.genre === "romance_aruaru" ? 90 : 84, total: row.score },
    ...
    return {
      ...
      male_attention_score: detail.male_attention_score,
      maleAttentionScore: detail.maleAttentionScore,
      dm_trigger_score: detail.dm_trigger_score,
      dmTriggerScore: detail.dmTriggerScore,
      comment_trigger_score: detail.comment_trigger_score,
      commentTriggerScore: detail.commentTriggerScore,
