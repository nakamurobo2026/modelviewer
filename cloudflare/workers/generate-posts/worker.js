...
function sourceLabel(type) {
  const labels = { threads: "Threads実投稿", x: "X投稿傾向", tiktok: "TikTok構文", instagram: "Instagram Reels", google_trends: "Google Trends", yahoo_realtime: "Yahoo!リアルタイム", reddit: "Reddit", hatebu: "はてなブックマーク", note: "note", togetter: "Togetter", news: "ニュース", blog: "ブログ", official: "公式サイト", wikipedia: "Wikipedia", manual: "手動メモ", local: "保存DB/ローカル" };
...
function inferSourceType(source) {
  ...
  let host = "";
  try { host = new URL(text.match(/https?:\/\/\S+/)?.[0] || text).hostname.replace(/^www\./, ""); } catch { host = ""; }
...
function makeSourceRecord(source, index) {
  ...
  return { id: `source-${index}`, sourceType, priority: priority.priority, weight: priority.weight, url, title: content.slice(0, 42) || sourceLabel(sourceType), content, reason: priority.reason, reliability, impact: Math.round(reliability * priority.weight), buzzElements: [/レジ|BGM|音/.test(content) ? "音の違和感" : "具体描写", /古い|昭和|懐|蛍光灯/.test(content) ? "懐かしさ" : "共感の余白"] };
}
...
function localIdeas(theme, category, count = IDEA_COUNT) {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return { text: text.slice(0, 90), category, score: clamp(68 + (index * 7) % 25, 60, 94), hook: pick(["共感", "違和感", "懐かしさ", "余白", "不穏"], index) };
  });
}
...
function localResearch(sources, persona = "違和感ノート") {
  ...
  const hooks = [first.match(/(閉店前|17時過ぎ|夕方|深夜|雨の日)/)?.[0] || "17時過ぎ", first.match(/(スーパー|ホームセンター|コンビニ|地方駅|商店街|ドラッグストア|市役所)/)?.[0] || "身近な場所"];
  const phrases = sources.map((source) => source.replace(/https?:\/\/\S+/g, "").replace(/[「」『』"']/g, "").trim().slice(0, 34)).filter(Boolean).slice(0, 8);
  const buzzElements = [text.includes("レジ") ? "レジ音だけ残る" : "音が少なくなる瞬間", text.includes("駐車場") ? "駐車場が急に広く見える" : "人が少ない場所", text.includes("蛍光灯") ? "古い蛍光灯の白さ" : "棚の色が暗く見える", "普通なのに少しだけずれる"];
  return { success: true, source: "worker-fallback", summary: `${hooks.join("の")}を起点に、具体描写と少しの違和感で投稿化する。`, buzzElements, hooks, phrases, patterns: ["共感型", "違和感型", "余白型"], sourceRecords, recommendedPostAngles: [`${persona}として、音や光から入る`, "元投稿の言い回しは避け、場所と時間を変えて観察にする", "コメントしたくなる余白を残して断定しすぎない"] };
}
...
async function handleGenerate(request, env) {
  ...
  const prompt = ["Threads向け投稿案を50件作る。", "AIっぽい抽象語は禁止。20〜90文字中心。コメントしたくなる余白を残す。", "場所、時間、音、光、人の少なさ、店舗の挙動を使う。", `カテゴリ:${category}`, `素材:${theme}`].join("\n");
  try {
...
async function handleResearch(request, env) {
  ...
  const prompt = ["入力されたURLメモ、コピペ投稿、メモ、キーワードからThreads運用向けのバズ要素を抽出する。", "元投稿の丸写しは禁止。言い回しは変換して使う。", "返すキー: summary, buzzElements, hooks, phrases, patterns, recommendedPostAngles", "sourceRecordsも返す。sourceType, priority, weight, url, title, content, reason, reliability, impactを含める。", "Priority S/Aを優先し、B/Cは背景か補助として扱う。", `人格:${persona}`, `sources:${JSON.stringify(sources)}`].join("\n");
  try {
    ...
    return json({ success: true, source: "openai", model, elapsedMs: Date.now() - startedAt, ...localResearch(sources, persona), ...parsed }, env, request);
  } catch (error) {
